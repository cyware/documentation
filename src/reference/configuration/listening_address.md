...
Cyware, by default, listens on the IP address `127.0.0.1` and port `8080`. This means that Cyware will only be accessible from the same machine it is running on.
...
::: warning
Please note that if you change the listening address to something other than 127.0.0.1, Cyware will be accessible from any machine on the network, so it is important to consider the security implications of doing so.
:::
